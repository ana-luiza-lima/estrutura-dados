...
//        public void adicionarContatoEmLote(Contato[] contatos ){
//            if(contatos.length >= )
...
//        }
        ...
        public String buscarContato(String nome){
            for(int i = 0; i < tamanho; i++){
                if(contatos[i].getNome().equals(nome)){
                    return "Nome: " + contatos[i].getNome() + "/n" +
                            "Telefone: " + contatos[i].getTelefone() + "/n" +
                            "Email: " + contatos[i].getEmail() + "/n";
                }
            }
        ...
        public void imprimir(){
            if(tamanho > 0){
                ...
                for (int i =0; i< contatos.length; i++){
                    if(contatos[i] != null){
                        IO.println(contatos[i].exibirContato());
                        IO.println("---------------------------");
                    }
                    if(i < tamanho-1){
                        System.out.print(", ");
